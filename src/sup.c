...
uint8_t sup_init(uint32_t u32baudrate)
{
...
uint8_t sup_init_module(uint8_t module_nr,sup_module_t* module)
{
	module_array[module_nr] = module;
	return SUP_SUCCESS;
}

uint8_t sup_get_properties(uint8_t* pu8properties, uint8_t u8module_nr, uint8_t u8reg_nr)
{
	if(u8module_nr >= SUP_MAX_MODULE_COUNT)
	{
		return SUP_ERROR_NO_MODULE;
	}
...
uint8_t sup_get_type(uint8_t* pu8type, uint8_t u8module_nr, uint8_t u8reg_nr)
{
	if(u8module_nr >= SUP_MAX_MODULE_COUNT)
	{
		return SUP_ERROR_NO_MODULE;
...
uint8_t sup_send(uint8_t* u8dataToSend_ptr, uint16_t u16length)
{
...
uint8_t sup_receive(uint8_t* u8rawData_ptr, uint16_t u16rawDataLength)
{
	int8_t u8retVal;
	...
	u8retVal = sup_ll_receive(au8data,&u16dataLength,u8rawData_ptr,u16rawDataLength);

	if(u8retVal == SUP_SUCCESS)
	{
		switch(au8data[0])
		{
		case WRITE_SYS:
		...
		while(sup_send_isbusy())
		{
			...
			if(u32temp < u32timeout_hold)
			{
				if((0xFFFFFFFF - u32timeout_hold - u32temp + 1) >= 50)
				{
					...
					return SUP_ERROR_USART_TIMEOUT;
				}
			}
			else
			{
		...
		u8retVal = sup_send(au8data,u16dataLength);
		return u8retVal;
	}
	return u8retVal;
}
